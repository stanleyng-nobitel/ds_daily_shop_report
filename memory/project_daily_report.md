...
- 消化売上(税抜) ＋ 小ラベル「ｽﾄﾚｯﾁ+指名含む」 / 会員費 / 会員数
- 新規来客者数 / 新規入会者数 / 新規入会率
- 退会者数 / 退会率
- 来客周期（MTDのみ）/ アクティブ率
...
| 2026-04-08 | — | ✓ 4/4データ: TDが再実行で正常化（既存110/既存新店25/新店45）。データ品質チェック + Flex Messageアラート動作確認済み |
...
| 項目 | 状態 |
|------|------|
| Schedule実行の正常動作確認（CAST書き換え後） | ✓ 4/8確認済み（TD再実行後に3種類正常）|
| TD側 shop_group 崩れ問題の解消確認 | ✓ TD担当者が再実行→4/4データ正常化確認済み（2026-04-08） |
| TD → BQ Export 設定 | 未完了 |
